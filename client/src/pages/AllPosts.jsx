...
export default function AllPosts() {
  ...
  useEffect(() => {
    const fetchPosts = async (page) => {
      const res = await fetch(`/api/post/getposts?page=${page}&limit=${postsPerPage}`);
  ...
  return (
    <div className="p-3">
      <div className="flex justify-between items-center p-3">
        <p className='text-3xl mx-auto font-semibold text-center'>Post Categories</p>
        <button
    ...
    return (
      ...
      <div className='group relative w-full sm:w-1/2 md:w-[130px]  border border-red-400 hover:border-2 h-[180px] overflow-hidden rounded-lg transition-all '>
        <Link to={`/posts?category=${encodeURIComponent(category)}`}>
